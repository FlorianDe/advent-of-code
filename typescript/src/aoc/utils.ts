...
export namespace Files {
  export const readContent = (year: number | string | Year, day: number | string | Day, isSample: boolean = false): string => {
    assertValidYear(year);
    assertValidDay(day);
    const fileName = `day${String(day).padStart(2, '0')}${isSample ? '_sample' : ''}.txt`;
